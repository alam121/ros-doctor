# ROS Doctor Diagnosis

- Inferred ROS version: `unknown`
- Files scanned: `3`

## Agent Verification Loop

1. Collected 3 readable evidence files from cases/ros2_tf_missing_frame.
2. Inferred ROS version as unknown.
3. Generated 10 candidate root-cause hypotheses from the rule library.
4. Verified candidates against confirming, supporting, and refuting evidence.
5. Rejected 6 candidates with no supporting evidence.
6. Ranked remaining hypotheses by evidence strength and contradiction penalties.

## Ranked Hypotheses

### 1. TF frame, timestamp, or simulation clock failure

- Score: `0.72` (medium confidence)
- Likely root cause: Transforms are unavailable or rejected because frame IDs, timestamps, or use_sim_time settings are inconsistent.
- Recommended fix: Verify the TF tree, frame names, /clock publication, and use_sim_time on all nodes that consume transforms.
- Verification checks:
  - `ros2 run tf2_tools view_frames`
  - `ros2 topic echo /tf --once`
  - `ros2 param get <node_name> use_sim_time`
  - `ros2 topic echo /clock --once`

#### Confirming Evidence
- `cases/ros2_tf_missing_frame/logs/navigation.log:1` [confirm:Timed out waiting for transform] [WARN] [controller_server]: Timed out waiting for transform from base_link to map
- `cases/ros2_tf_missing_frame/logs/navigation.log:2` [confirm:Invalid frame ID] [ERROR] [bt_navigator]: Invalid frame ID "map" passed to canTransform argument target_frame - frame does not exist

#### Supporting Context
- `cases/ros2_tf_missing_frame/config/nav2.yaml:3` [context:\bmap\b] global_frame: map
- `cases/ros2_tf_missing_frame/config/nav2.yaml:4` [context:\bbase_link\b] robot_base_frame: base_link
- `cases/ros2_tf_missing_frame/frames.txt:1` [context:\bodom\b] Frame odom exists with parent world.
- `cases/ros2_tf_missing_frame/frames.txt:2` [context:\bbase_link\b] Frame base_link exists with parent odom.
- `cases/ros2_tf_missing_frame/frames.txt:2` [context:\bodom\b] Frame base_link exists with parent odom.
- `cases/ros2_tf_missing_frame/frames.txt:3` [context:\bmap\b] Frame map is missing.

### 2. ROS2 topic or namespace mismatch

- Score: `0.24` (low confidence)
- Likely root cause: Publisher and subscriber names do not resolve to the same fully qualified topic, often because remaps, namespaces, or leading slashes differ.
- Recommended fix: Normalize topic names and remaps across launch files and node code. Verify with ros2 topic list, ros2 node info, and a minimal pub/sub test.
- Verification checks:
  - `ros2 node list`
  - `ros2 node info <node_name>`
  - `ros2 topic list -t`
  - `ros2 topic echo <expected_topic> --once`

#### Confirming Evidence
- `cases/ros2_tf_missing_frame/logs/navigation.log:1` [confirm:Timed out waiting for transform] [WARN] [controller_server]: Timed out waiting for transform from base_link to map

### 3. ROS2 parameter name or type mismatch

- Score: `0.04` (low confidence)
- Likely root cause: A launch file or YAML config provides a parameter name or value type that does not match what the node declares or expects.
- Recommended fix: Compare the node's declared parameters with YAML and launch inputs, correct the name or type, and verify with ros2 param describe/get.
- Verification checks:
  - `ros2 param list <node_name>`
  - `ros2 param describe <node_name> <parameter>`
  - `ros2 param get <node_name> <parameter>`
  - `ros2 launch <package> <launch_file> --show-args`

#### Confirming Evidence
- No direct evidence captured.

#### Supporting Context
- `cases/ros2_tf_missing_frame/config/nav2.yaml:2` [context:ros__parameters] ros__parameters:

### 4. URDF or xacro parse failure

- Score: `0.04` (low confidence)
- Likely root cause: Robot description generation fails because a URDF/xacro file has malformed XML, an undefined xacro property/macro, or an invalid mesh/reference path.
- Recommended fix: Run xacro directly, fix the reported XML/property/path issue, then restart robot_state_publisher and verify the robot_description parameter.
- Verification checks:
  - `xacro <robot>.urdf.xacro`
  - `check_urdf <robot>.urdf`
  - `ros2 param get /robot_state_publisher robot_description`
  - `ros2 run tf2_tools view_frames`

#### Confirming Evidence
- No direct evidence captured.

#### Supporting Context
- `cases/ros2_tf_missing_frame/logs/navigation.log:3` [context:robot_state_publisher] [INFO] [robot_state_publisher]: publishing fixed transforms for base_link and laser

